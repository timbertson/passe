...
module Make (Client:Cohttp_lwt.S.Client) = struct
	...
	let read_for_writing t path (consumer: Lock.proof -> (string option, Error.t) result -> 'a Lwt.t) : 'a Lwt.t =
		...
		PathLock.acquire path (fun proof ->
			let response = post t ~body url
				|> LwtMonad.bind allowing_404
				|> LwtMonad.bind to_standard_error in

			let contents: (string option, Error.t) Lwt_r.t = response |> Lwt_r.bind (function
	...
	let read t path : (string option, Error.t) Lwt_r.t = read_for_writing t path
		(fun _ contents -> Lwt.return contents)

	let mutations lst =
		`Assoc ["mode", `String "NON_TRANSACTIONAL"; "mutations", `List lst]

	let write t path ?proof contents =
	...
	let connect = function
		| Cloud_datastore key_file ->
			...
			let ctx =
				...
				in
				let dns: Dns.t = Dns.create (Happy_eyeballs_lwt.create ()) in
				let resolve svc uri = (match Uri.host uri with
					| None -> Lwt.return (`Unknown "No host")
					| Some host ->
						let domain = host |> Domain_name.of_string_exn |> Domain_name.host_exn in
						Dns.gethostbyname dns domain |> Lwt.map (function
							| Ok ipaddr -> `TCP (Ipaddr.V4 ipaddr, svc.Resolver.port)
							| Error (`Msg msg) -> `Unknown (Printf.sprintf "Couldn't resolve host %s: %s" host msg)
						)
				) in
